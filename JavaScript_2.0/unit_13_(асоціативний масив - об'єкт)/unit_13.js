...
function f6() {
	let i1 = document.querySelector('.i-61').value
	let i2 = document.querySelector('.i-62').value
	a6[i1] = i2
...
function f15() {
	let out = ''
	for (let key in a15) {
		for (let t in a15[key]) out += a15[key][t] + ' '
	}
...
function f18() {
	let out = ''
	let b = document.querySelector('.i-18').value
	if (a18[b] !== undefined) {
		console.log(a18[b])
	}
	// if (a18.hasOwnProperty(b)) {
	//     console.log(a18[b]);
	// }
	for (let i = 0; i < a18[b].length; i++) {
		out += a18[b][i] + ' '
	}
...
function f19() {
	let out = ''
	let b = document.querySelector('.i-19').value
	for (let key in a19) {
		for (let i = 0; i < a19[key].length; i++) {
			if (a19[key][i].toLowerCase() == b.toLowerCase()) {
				out = key
				break
			}
		}
