...
function t7() {
	randomI = Math.floor(Math.random() * a7.length)
	randomL = a7[randomI]
	document.querySelector('.out-7').textContent += randomL
}
...
function t11(event) {
	...
	for (let i = 0; i < divs11.length; i++) {
		divs11[i].classList.remove('active')
	}
	let key = event.key
	if (key === ' ') key = 'space'
	if (key === 'Control') key = 'ctrlLeft'
	if (key === 'Alt') key = 'altLeft'
	if (key === 'Enter') key = 'enter'
	let keyActive = document.querySelector(`.keyboard[data="${key}"]`)
	if (keyActive) keyActive.classList.add('active')
}
