...
function t1(elem) {
	...
	function r1(element) {
		...
		Array.from(children).forEach(elem => r1(elem));
		//console.log(out);
	}
...
function t3(elem) {
	...
	function r3(element) {
		let children = element.children;
		if (children.length === 0) out += element.textContent;
		Array.from(children).forEach(elem => r3(elem));
	}
...
function t4(elem) {
	...
	function r4(element) {
		let children = element.children;
		if (children.length === 0) element.style.background = 'orange';
		Array.from(children).forEach(elem => r4(elem));
	}
}
...
function t5(elem) {
	...
	function r5(element) {
		...
		Array.from(children).forEach(elem => r5(elem));
	}
}
