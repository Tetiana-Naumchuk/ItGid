...
function t11() {
	let res = Array.from(arguments).reduce((accum, item) => {
		accum += item;
		return accum;
	}, 0);
	return res;
...
function t12(...args) {
	let res = args.reduce((accum, item) => {
		accum += item;
		return accum;
	}, 0);
	return res;
...
function t17() {
	let v = document.querySelector('.s-171').value;
	return v;
}
...
function t21(s) {
	while (true) {
		let res = t13(10, 20);
		if (s === 'even' && res % 2 === 0) return res;
		if (s === 'odd' && res % 2 !== 0) return res;
	}
}
...
function t26() {
	...
	return (out.textContent = i.toLowerCase());
}
