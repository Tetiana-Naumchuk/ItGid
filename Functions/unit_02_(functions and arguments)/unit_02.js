...
function t1(min, max) {
	...
	document.querySelector('.out-1').textContent = random;
}
...
function t3(min = 0, max = 100) {
	let random = Math.floor(Math.random() * (max - min + 1) + min);
	document.querySelector('.out-3').textContent = random;
}
...
function t10(...args) {
	document.querySelector('.out-10').innerHTML = args.length;
}
...
function t11() {
	let sum = Array.from(arguments).reduce((accum, item) => {
		accum += item;
		return accum;
	}, 0);
...
function t12(...args) {
	let sum = args.reduce((accum, item) => {
		accum += item;
		return accum;
	});
